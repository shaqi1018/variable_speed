...
EXAMPLE_FILE = "O-A-1.mat"

# LPS算法参数
LPS_CONFIG = {
    'nperseg': 131072,          # STFT窗口长度
    'min_freq': 5,              # 最小搜索频率 (Hz)
    'max_freq': 35,             # 最大搜索频率 (Hz)
    'c1': 0.9,                  # 自适应下界系数
    'c2': 1.1,                  # 自适应上界系数
...
if __name__ == "__main__":
    ...
    t, if_estimated, f, Zxx = entropy_based_lps(
        signal_data, FS,
        **LPS_CONFIG,
        verbose=True
    )
    ...
    print(f"\n转速范围: {rpm.min():.1f} - {rpm.max():.1f} RPM")

    # 绘制IF结果（非阻塞）
    plot_if_extraction_result(
        t, if_estimated, f, Zxx,
        max_display_freq=40,
